...
From the admin dashboard you can create and revoke API keys. Keys use the format `mk_` + 32 random characters. The full key is shown once at creation and stored as a SHA-256 hash. The legacy `MIRAGE_API_KEY` env var still works as a fallback.
...
| `MIRAGE_API_KEY` | No | — | Legacy API key for session creation (open if unset) |
