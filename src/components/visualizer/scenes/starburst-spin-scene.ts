...
/**
 * Starburst Spin Scene
 *
 * Full-screen starburst rays that cover every pixel — no black, ever.
 * The centred texture rotates slowly on its Y axis for a 3D spinning effect.
 * Rays radiate outward from the texture centre and rotate with audio.
 */
...
export class StarburstSpinScene {
  ...
  constructor(
    ...
    // Centre texture plane — renders in front of starburst, with double-sided
    // material so the texture is visible during Y-axis rotation
    ...
    this.logoMaterial = new THREE.ShaderMaterial({
      ...
      transparent: true,
      depthWrite: false,
      side: THREE.DoubleSide, // visible from both sides during Y rotation
    });
  ...
  update(bass: number, mid: number, high: number): void {
    ...
    // Y-axis rotation: slow continuous spin (~8 seconds per revolution)
    const spinSpeed = this.config.animationSpeed * 0.8;
    this.logoMesh.rotation.y = time * spinSpeed;
...
const METADATA: SceneRegistration = {
  id: 'starburst-spin',
  name: 'Starburst Spin',
  description: 'Full-screen rays with Y-axis spinning texture — no black, ever',
  category: 'immersive',
