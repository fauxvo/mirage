...
const VERTEX_SHADER = `
  ...
  void main() {
    ...
    float freq = 1.5 + uMid * 2.0;
    float amp = 0.3 + uBass * 1.2;

    float n = snoise(normal * freq + t * 0.3) * amp;
    n += snoise(normal * freq * 2.0 - t * 0.2) * amp * 0.5;
...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    float fresnelIntensity = 0.5 + uHigh * 1.5;
    fresnel *= fresnelIntensity;

    float dispMix = smoothstep(-0.3, 0.5, vDisplacement);
    vec3 baseColor = mix(uPrimary, uSecondary, dispMix);

    vec3 color = baseColor + uAccent * fresnel;
...
registerScene('orb', (scene, config) => new OrbScene(scene, config), METADATA);
